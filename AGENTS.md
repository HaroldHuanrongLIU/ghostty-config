...
- **`+layout.svelte` is the single color-key read funnel** — it resolves effective colors into the global `--config-*` CSS vars. Every preview/surface (chrome, `CursorPreview`, the DOM terminal) reads those CSS vars only, inheriting layered colors for free. Do not add new direct readers of `config.<colorkey>` or `effectiveColors()` in components; read the vars.
